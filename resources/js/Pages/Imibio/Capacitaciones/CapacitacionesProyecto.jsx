...
const { id } = props;
  ...
  const registrosFiltrados = registros.filter((registro) => registro.proyectoid === parseInt(id));
  const capacitaciones = registrosFiltrados.filter((registro) => registro.actividadid === 5);
      ...
      <div className="banner-container" style={{ position: 'relative' }}>
        {proyecto ? (
          <>
            ...
            />
            <h1 className="texto-banner">{proyecto.proyectonombre}</h1>
          </>
        ) : (
          <p className="text-center">Cargando datos del proyecto...</p>
        )}
      </div>
      <div className="m-3">
...
export default CapacitacionProyecto;
